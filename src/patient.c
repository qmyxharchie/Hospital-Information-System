...
static Patient* g_patientHead = NULL;  // 病人链表头指针
static Patient* g_patientTail = NULL;  // 病人链表尾指针
//--------------------
...
int modifyPatient(Patient* head, char* cardNo, PatientData newData){
    ...
    safeStringCopy(target->data.gender, newData.gender, 10);
    safeStringCopy(target->data.idCard, newData.idCard, 20);
    safeStringCopy(target->data.phone, newData.phone, 15);
    target->data.isActive = newData.isActive;
    rebuildPatientFile(head）;
    printf("患者信息修改成功！\n");
    return 1; 
}
