...
// 检查用户是否有执行特定操作的权限
int checkPermission(UserType user_type, const char* operation) {
...
// 检查用户是否可以访问特定患者数据
int canAccessPatientData(UserType user_type, int requesting_user_id, int target_patient_id) {
    switch (user_type) {
    case PATIENT:
        // 患者只能访问自己的数据
...
// 检查用户是否可以管理特定挂号
int canManageRegistration(UserType user_type, int requesting_user_id, int reg_patient_id) {
    switch (user_type) {
    case PATIENT:
        // 患者只能管理自己的挂号
...
// 其他权限检查函数的实现...
int canUpdateMedical_record(UserType user_type) {
    return (user_type == DOCTOR);
}

int canViewFinancialData(UserType user_type) {
    return (user_type == ADMIN);
}
