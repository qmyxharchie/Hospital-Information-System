...
> **提示**：以下四个模块的菜单实现逻辑与"病人管理菜单"完全相同，区别仅在于：
> - 调用的函数名不同（如 `addDoctor`、`addBed` …）
> - 输入的字段不同
> - 写回的文件不同（如 `rebuildDoctorFile()`）
> - **挂号模块因源文件尚未完成，此处暂略，待后续补充**

#### 医生管理菜单 — showDoctorManagement()
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 3:  // 修改
                ...
                d = findDoctorByEmpNo(g_doctorHead, empNo);
                if (d) {
                    printf("当前信息：%s %s %s 最大接诊%d 已接诊%d\n",
                           d->data.empNo, d->data.name, d->data.dept,
                           d->data.maxPatients, d->data.currentPatients);
                    printf("请输入新姓名: ");      scanf("%49s", name);
                    printf("请输入新科室: ");      scanf("%49s", dept);
                    printf("请输入新出诊时间: ");  scanf("%99s", schedule);
                    printf("请输入新最大接诊数: "); scanf("%d", &maxPatients);
                    DoctorData newData;
                    strcpy(newData.empNo, empNo);
                    strcpy(newData.name, name);
                    strcpy(newData.dept, dept);
                    strcpy(newData.schedule, schedule);
                    newData.maxPatients = maxPatients;
                    newData.currentPatients = d->data.currentPatients;
                    if (modifyDoctor(g_doctorHead, empNo, newData)) {
                        printf("[OK] 医生信息修改成功！\n");
                    }
                } else {
                    printf("[ERROR] 未找到该医生！\n");
            ...
            case 4:  // 查找
                printf("\n  1. 按工号查找  2. 按姓名查找  3. 按科室查找\n");
                printf("  请选择: ");
                int findChoice;
                scanf("%d", &findChoice);
                getchar();
                if (findChoice == 1) {
                    printf("请输入工号: "); scanf("%19s", empNo);
                    d = findDoctorByEmpNo(g_doctorHead, empNo);
                    if (d) {
                        printf("%-10s %-10s %-10s %-20s %-10s %-10s\n",
                               "工号", "姓名", "科室", "出诊时间", "最大接诊", "已接诊");
                        printf("%-10s %-10s %-10s %-20s %-10d %-10d\n",
                               d->data.empNo, d->data.name, d->data.dept,
                               d->data.schedule, d->data.maxPatients, d->data.currentPatients);
                    } else {
                        printf("[ERROR] 未找到！\n");
                    }
                } else if (findChoice == 2) {
                    printf("请输入姓名关键字: "); scanf("%49s", name);
                    result = findDoctorsByName(g_doctorHead, name);
                    if (result) {
                        listAllDoctors(result);
                        freeDoctorsResultChain(result);
                    } else {
                        printf("[ERROR] 未找到！\n");
                    }
            ...
            case 6:  // 科室统计
                printf("请输入科室名称: "); scanf("%49s", dept);
                int count = 0;
                getDeptStats(g_doctorHead, dept, NULL, &count);
                break;
...
#### 住院管理菜单 — showHospitalizationManagement()
...
void showHospitalizationManagement(void) {
    ...
    while (1) {
        printf("\n-------- 住院管理 --------\n");
        printf("1. 入院登记       4. 按住院单号查询\n");
        printf("2. 出院结算       5. 显示当前在院病人\n");
        printf("3. 按卡号查询     6. 显示所有住院记录\n");
        printf("0. 返回上级菜单\n");
        printf("请选择: ");
...
void refreshPatientTable(Table* table) {
    ...
    while (p != nullptr && row < 100) {
        table->setCellText(row, 0, p->data.cardNo);
        table->setCellText(row, 1, p->data.name);
        table->setCellText(row, 2, p->data.gender);
        table->setCellText(row, 3, p->data.phone);
        table->setCellText(row, 4, p->data.isActive ? "住院中" : "正常");
        p = p->next;
        row++;
    }
}
...
statDocBtn->setOnClickListener([&mainWindow, &g_doctorHead]() {
    ...
    while (d != nullptr) {
        sprintf(buf, "%s(%s): %d/%d人\n",
                d->data.name, d->data.dept, d->data.currentPatients, d->data.maxPatients);
        strcat(msg, buf);
        d = d->next;
    }
